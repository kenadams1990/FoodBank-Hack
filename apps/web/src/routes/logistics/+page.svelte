<script lang="ts">
  ...
  const columns = [
    { key: 'PENDING_APPROVAL',      label: 'Pending Approval',      color: 'border-t-brand' },
    ...
    { key: 'SHIPPED',               label: 'Shipped',               color: 'border-t-brand' },
...
<!-- Page header -->
<div class="mb-6">
  <h1 class="font-display font-bold text-2xl text-ink">Logistics Board</h1>
  <p class="font-mono text-xs text-ink/40 mt-1">Track every lot from surplus to delivery. Cold-chain rescue panel live below.</p>
</div>
...
<div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
  ...
  <div class="xl:col-span-3">
    {#if loading}
      <p class="font-mono text-xs text-ink/30 animate-pulse">Loading board…</p>
    {:else}
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {#each columns as col}
          <div class="bg-white rounded-xl border border-line border-t-4 {col.color} p-4 min-h-48">
            <h3 class="font-mono text-[10px] uppercase tracking-[0.15em] text-ink/40 mb-3">
              {col.label}
              <span class="ml-1 bg-surface text-ink/50 rounded px-1.5 py-0.5">
            ...
            <div class="space-y-2">
              {#each (kanban[col.key] ?? []) as lot}
                ...
                >
                  <p class="font-display text-sm font-semibold capitalize text-ink">{lot.species}</p>
                  <p class="font-mono text-[11px] text-ink/40 mt-0.5">
                    {lot.lbs.toLocaleString()} lbs
                  </p>
                  {#if lot.score !== undefined}
                    <span class="font-mono text-[10px] text-brand mt-1 inline-block">{lot.score}/100</span>
                  {/if}
                </a>
              {/each}
              {#if (kanban[col.key] ?? []).length === 0}
                <p class="font-mono text-[10px] text-ink/20 text-center pt-4">No lots here</p>
              {/if}
            </div>
          </div>
        {/each}
